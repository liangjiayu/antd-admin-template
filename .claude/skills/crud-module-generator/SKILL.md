...
根据用户提供的自然语言描述，生成完整的 CRUD 模块代码。生成的代码遵循项目现有 `app/components/crud-table/` 的参考实现模式。
...
export function use{Entity}ConfigModal(params?: {
  /** 弹窗关闭回调 */
  handleOnClose?: () => void;
  /** 弹窗完成回调 */
  handleOnFinish?: () => void;
...
- API 服务通过 `@umijs/openapi` 自动生成，统一从 `@/services` 导入：`import { FastApiServices } from '@/services'`，调用形式 `FastApiServices.{Module}.{method}(...)`
- 类型定义在全局命名空间 `FastAPI` 下（如 `FastAPI.Task`），无需手动 import
- 参考实现位于 `app/components/crud-table/`（包含 `index.tsx`、`constants.ts`、`components/task-config-modal.tsx`），可直接对照
